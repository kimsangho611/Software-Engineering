  ...
  try {
    if (token_res === 401) {
      ...
      if (type === "1") {
        ...
        const result = await connection.query(
          "select p.*, count(*) as likecnt from product p join (select pi.Product_p_id from user u join productinquiry pi on u.u_id = pi.User_u_id where u.u_id = ?) u_pi on p.p_id = u_pi.Product_p_id;",
          [token_res.uid]
        );
        res.status(200).send({ success: true, result: result[0] });
      } else {
