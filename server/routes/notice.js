...
router.get("/list", async function (req, res) {
  ...
  try {
    const result = await connection.query(
      "select post_id, post_title, post_date, post_view from announce;"
    );
...
router.get("/detail", async function (req, res) {
  ...
  try {
    await connection.query(
      "update announce set post_view= post_view+1 where post_id=?;",
      noticeId
    );
    const result = await connection.query(
      "select * from announce where post_id = ?;",
      [noticeId]
    );
    res.status(200).send({ success: true, result: result[0] });
  } catch (err) {
...
router.post("/upload", async function (req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .send({ success: true, msg: "공지사항 등록이 완료되었습니다." });
  } catch (err) {
    res.status(500).send({ success: false, msg: "서버 오류" + err });
  } finally {
    connection.release();
...
router.post("/:noticeId/edit", async function (req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .send({ success: true, msg: "공지사항 수정이 완료되었습니다." });
  } catch (err) {
    res.status(500).send({ success: false, msg: "서버 오류" + err });
  } finally {
    connection.release();
...
router.delete("/:noticeId/delete", async function (req, res) {
  ...
  const connection = await pool.getConnection(async (conn) => conn);
  try {
    await connection.query("delete from announce where post_id = ?;", [
      noticeId,
    ]);
