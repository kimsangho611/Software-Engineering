...
router.get("/list", async function (req, res) {
  ...
  try {
    const result = await connection.query(
      "select q.q_id, q.q_title, q.q_date, q.q_answer from user u join questions q on u.u_id = q.User_u_id order by q.q_id DESC;"
    );
...
router.post("/:inquiryId/upload", async function (req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .send({ success: true, msg: "문의사항 답변 등록을 완료했습니다." });
  } catch (err) {
    res.status(500).send({ success: false, msg: "서버 오류" + err });
  } finally {
    connection.release();
