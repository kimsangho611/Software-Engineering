...
router.get("/seller/:productId", async function (req, res) {
  var productId = req.params.productId;
  const connection = await pool.getConnection(async (conn) => conn);
  try {
    const result = await connection.query(
    ...
    const result2 = await connection.query(
      "select * from review where User_u_id = ?;",
      [u_id]
    );

    const result3 = await connection.query(
      "select ifnull(sum(re_star) / count(*), 0) as u_star\
      from review\
      where User_u_id = ?;",
      [u_id]
    );
    res.status(200).send({ list: result2[0], avg: result3[0][0] });
  } catch (err) {
    res.status(500).send({ success: false, msg: "서버 오류" + err });
  } finally {
    connection.release();
